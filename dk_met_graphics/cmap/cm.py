...
def ncl_cmaps(name):
    ...
    cmap_file = pkg_resources.resource_filename(
        "dk_met_graphics", "resources/colormaps_ncl/" + name + '.rgb')
...
def guide_cmaps(name):
    ...
    # color map file directory
    cmap_file = pkg_resources.resource_filename(
        "dk_met_graphics", "resources/colormaps_guide/cs" + str(name) + '.txt')
    if not os.path.isfile(cmap_file):
        return None
...
class Cmaps(object):
    ...
    :Examples:
    >>> from dk_met_graphics.color_maps import Cmaps
    >>> import matplotlib.pyplot as plt
    >>> import numpy as np
    ...
    def _listfname(self):
        cmapsfile_dir = pkg_resources.resource_filename(
            "dk_met_graphics", "resources/colormaps_ncl")
